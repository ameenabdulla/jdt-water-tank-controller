        ...
        lights.forEach((light, i) => {
            ...
            card.innerHTML = `
                <div class="light-card-header">
                    <span class="light-conn-badge">● ONLINE</span>
            ...
            function applyState(isOn, sendWs = true) {
                ...
                if (sendWs && window.ESP32WS) {
                    window.ESP32WS.send({ type: 'light', id: light.id - 1, state: isOn });
                }
            }

            input.addEventListener('change', (e) => {
                applyState(e.target.checked);
                ActivityTimeline.addEvent(`${light.name} light turned ${light.on ? 'ON' : 'OFF'}`, 'light');
            });
            ...
            // Cards are created dynamically after ScrollAnimations.init(),
            // so the IntersectionObserver never sees them.
            // Force-reveal with a short stagger so the slide-in still plays.
...
const GateControl = (function() {
    ...
    function updateVisuals() {
        ...
        let newStatus = '';
        if (state.position === 0) newStatus = 'CLOSED';
        ...
        if (state.status !== newStatus) {
            ...
            if (statusText) {
                ...
                if (state.status === 'OPEN') statusText.classList.add('status-open');
            ...
            if (navInd) {
                navInd.textContent = state.status;
    ...
    function setTarget(target, sendWs = true) {
        ...
        if (sendWs && window.ESP32WS) {
            if (target === 100) window.ESP32WS.send({ type: 'gate', action: 'open' });
            else if (target === 0) window.ESP32WS.send({ type: 'gate', action: 'close' });
    ...
    function setHolding(direction) {
        ...
        if (window.ESP32WS) {
            if (direction > 0) window.ESP32WS.send({ type: 'gate', action: 'hold_open' });
            else if (direction < 0) window.ESP32WS.send({ type: 'gate', action: 'hold_close' });
            else window.ESP32WS.send({ type: 'gate', action: 'hold_stop' });
        }
    }
    ...
    function init() {
        ...
        if (openBtn) openBtn.addEventListener('click', () => {
            const allowed = window.SafetyMonitor ? window.SafetyMonitor.requestGateOpen(() => setTarget(100)) : true;
            if (allowed) setTarget(100);
        });
        if (closeBtn) closeBtn.addEventListener('click', () => setTarget(0));

        const startHoldOpen = (e) => {
            e.preventDefault();
            const allowed = window.SafetyMonitor ? window.SafetyMonitor.requestGateOpen(() => setHolding(1)) : true;
            if (allowed) setHolding(1);
        };
        const startHoldClose = (e) => { e.preventDefault(); setHolding(-1); };
    ...
    function init() {
        ...
        if (autoBtn && manualBtn && manualControls) {
            autoBtn.addEventListener('click', () => {
                ...
                ActivityTimeline.addEvent('Pump set to AUTO mode', 'pump');
                if (window.ESP32WS) window.ESP32WS.send({ type: 'pump', mode: 'auto' });
                if (window.WaterTank) {
                    const ws = window.WaterTank.getState();
                    const st = Settings.getState();
                    checkAuto(ws.level, st.lowThreshold, st.highThreshold);
                }
            });
            ...
            manualBtn.addEventListener('click', () => {
                ...
                if (window.ESP32WS) window.ESP32WS.send({ type: 'pump', mode: 'manual' });
            });
        }
        ...
        if (powerBtn) {
            powerBtn.addEventListener('click', () => {
                if (state.mode === 'manual') {
                    ...
                    setOn(nextOn, 'manual');
                    if (window.ESP32WS) window.ESP32WS.send({ type: 'pump', mode: 'manual', on: nextOn });
                }
            });
        }
    }
...
const SafetyMonitor = (function() {
    ...
    function setSensorState(sensorId, status, skipModal) {
        ...
        if (sensorId === 'ir' && status === 'danger' && !skipModal) {
            showGateModal('Intruder / Object detected near gate perimeter');
        }
    }
    ...
    // Returns true if gate open is allowed immediately, false if intercepted
    function requestGateOpen(actionCallback) {
        if (sensors.ir.status === 'safe' && sensors.gas.status !== 'danger' && sensors.flame.status !== 'danger') {
            return true; // all clear — allow immediately
        }
        // Store the callback and show modal
    ...
    return { init, setSensorState, requestGateOpen, demo, stopAutoDemo, applyServerState };
})();
